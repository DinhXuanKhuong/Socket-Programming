...
def create_packet(data, ip, port, type, connection):
...
def create_packet_tcp(data, ip, port, type, connection):
    
    # print(ip, port)
    protocol_name = b'GTCP'  # Định dạng protocol name (4 byte)
    # sender_ip = fill_zero(ip_to_bytes(ip), 4)  # Địa chỉ IP (4 byte)
    # sender_port = long_to_bytes(port, 2)  # Port (2 byte)
    # type_request = type.encode('utf-8')[:1]  # Loại request (1 byte)
    # content = data if isinstance(data, bytes) == True else data.encode('utf-8')  # Nội dung (n bytes)
    ...
    # header = protocol_name + sender_ip + sender_port + type_request + content_length
    header = protocol_name + content_length
   
    # Tạo gói tin
    packet = header + data
    return packet

def GTCP_header_parse(header):
    protocol_name = header[0:4]
    ...
    content_length = bytes_to_long(header[4:8])
    # if content_length > 1024:
    #     print(content_length)
    # print(content_length, protocol_name)
    return protocol_name, content_length
