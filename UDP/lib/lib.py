...
def create_packet_udp(data, chunk_id): 
    ...
    checksum = calculate_hash_sha256_checksum(data) # Checksum (32 byte)
    content_length = long_to_bytes(len(data), 4)  # Độ dài nội dung (4 byte)
    header = protocol_name + chunk_id + checksum + content_length # Header (44 byte)
   
    # Tạo gói tin
    packet = header + data
    return fill_zero(packet, 2048) # (header = 44 bytes) + content + 0-bytes padding = 2048 bytes
